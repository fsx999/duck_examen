...
def get_etudiant_pagine(qs, nb_amphi, nb_table):
    t = []
    result = paginator_etudiant(qs, nb_amphi)
    for x in result:
...
def paginator_etudiant(qs, nb):
    l = len(qs) / nb
    pa = NamePaginator(qs, on='cod_ind', per_page=l)
    if pa.num_pages > nb:
        a = pa.pages.pop(-1)
        pa.pages[-1].add(a.object_list)
        pa.pages[-1].letters.extend(a.letters)
    return pa.pages